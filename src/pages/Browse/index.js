...
const Browse = () => {
  ...
  const filteredRecords = records.filter(record => {
    if( (genre.some(item => record.genre.includes(item)) || genre.length === 0) 
    && (style.some(item => record.style.includes(item)) || style.length === 0)
    && (year.some(item => record.year === parseInt(item)) || year.length === 0)
    && (artist.some(item => record.artist.includes(item)) || artist.length === 0) ) {
      return true
    } return false
  })
  ...
  return (
    <>
      <div style={{display: 'inline'}}>
        <MultipleSelect options={genreArray} label={'genre'} updateFilter={updateFilter}/>
        <MultipleSelect options={styleArray} label={'style'} updateFilter={updateFilter}/>
        <MultipleSelect options={yearArray} label={'year'} updateFilter={updateFilter}/>
        <MultipleSelect options={artistArray} label={'artist'} updateFilter={updateFilter}/>
      </div>
      <div className='body'>
        {records.length === 0 ? <p>You currently have no records on your shelf</p>
        : filteredRecords.length === 0 ? <p> No records match your filters </p>
